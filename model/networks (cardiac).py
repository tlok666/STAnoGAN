    ...
    def __init__(self, isize, nz, nc, ndf, ngpu, n_extra_layers=0, add_final_conv=True):
        ...
        if add_final_conv:
            main.add_module('final-{0}-{1}-conv'.format(cndf, 1),
                            nn.Conv3d(cndf, nz, 4, 1, 0, bias=False))
...
class BasicDiscriminator(nn.Module):
    ...
    def __init__(self, opt):
        ...
        feat.add_module('final-{0}-{1}-conv'.format(cndf, 1),
                            nn.Conv3d(cndf, nz, 4, 1, 0, bias=False))
        clas.add_module('classifier', nn.Conv3d(nz, 1, 3, 1, 1, bias=False))
...
class Unet_Attention(nn.Module):
    def __init__(self, config):
        ...
        self.netG = define_G(config, norm='batch', use_dropout=False, init_type='normal')
        self.Conv_lstm1 = _conv_lstm(in_dim = config.iChanel)
        self.ResBlock1 = Residual_block(in_dim = config.iChanel)
        self.ResBlock2 = Residual_block(in_dim = config.iChanel)
        self.ResBlock3 = Residual_block(in_dim = config.iChanel)
        self.ResBlock4 = Residual_block(in_dim = config.iChanel)
        self.attention_map_list = []
        self.Cell_state = torch.zeros(1, 1, int(config.iDepth), int(config.iWidth), int(config.iHeight)).cuda()
    ...
    def forward(self, x):
        ...
        for i in range(3):
            ...
            lstm_ret = self.Conv_lstm1(x1, self.Cell_state)
            self.Cell_state = lstm_ret['cell_state']
            self.attention_map_list.append(lstm_ret['attention_map'])
        x = lstm_ret['attention_map']*x
        x_= self.netG(x)
        return x, x_, self.attention_map_list
...
class _conv_lstm(nn.Module):
    ...
    def __init__(self, in_dim = 1):
        super(_conv_lstm,self).__init__()       
        self.conv_i = nn.Conv3d(in_dim, 1, kernel_size=3, stride=1, padding=1, bias=False)
        self.sigmoid_i = nn.Sigmoid()

        self.conv_f = nn.Conv3d(in_dim, 1, kernel_size=3, stride=1, padding=1, bias=False)
        self.sigmoid_f = nn.Sigmoid()
        self.tan = nn.Tanh()

        self.convCell = nn.Conv3d(in_dim, 1, kernel_size=3, stride=1, padding=1, bias=False)
        
        self.conv_o = nn.Conv3d(in_dim, 1, kernel_size=3, stride=1,padding=1, bias=False)
        self.sigmoid_o = nn.Sigmoid()


        self.featureConv = nn.Conv3d(1, 1, kernel_size=3, stride=1,padding=1, bias=False)
        self.featureSig = nn.Sigmoid()

    def forward(self, input_tensor, input_cell_state):
        x1 = self.conv_i(input_tensor)
        sigmoid_i  = self.sigmoid_i(x1)
        
        x2 = self.conv_f(input_tensor)
        sigmoid_f  = self.sigmoid_f(x2)
        
        cell_state = \
            sigmoid_f  * input_cell_state + \
            sigmoid_i  * self.tan(self.convCell(input_tensor))
            
        x3 = self.conv_o(input_tensor)
        x3_sig_o = self.sigmoid_o(x3)
        
        lstm_feats = x3_sig_o * self.tan(cell_state)
        attention_map = self.featureConv(lstm_feats)
        attention_map = self.featureSig(attention_map)
...
class Residual_block(nn.Module):                   
    def __init__(self, in_dim = 1):
        ...
        self.conv_1 = nn.Conv3d(in_dim, in_dim, kernel_size=3, stride=1, padding=1, bias=False)
        self.relu_1 = nn.ReLU()

        self.conv_2 = nn.Conv3d(in_dim, in_dim, kernel_size=3, stride=1, padding=1, bias=False)
        self.relu_2 = nn.ReLU()
        self.relu_3 = nn.ReLU()
        
    def forward(self, input_tensor):
        x1 = self.conv_1(input_tensor)
        x1 = self.relu_1(x1)
        ...
        x3 = input_tensor + x2
        return self.relu_3(x3)
    
def ncc(x, y):
    '''mean_x = torch.mean(x, [1,2,3], True)
    mean_y = torch.mean(y, [1,2,3], True)
    mean_x2 = torch.mean(x**2, [1,2,3], True)
    mean_y2 = torch.mean(y**2, [1,2,3], True)
    stddev_x = torch.sum(torch.sqrt(mean_x2 - mean_x**2), [1,2,3], True)
    stddev_y = torch.sum(torch.sqrt(mean_y2 - mean_y**2), [1,2,3], True)
    MeanMatrix = (x - mean_x) * (y - mean_y) / (stddev_x * stddev_y)
    MeanMatrix[ ~ torch.isfinite( MeanMatrix )] = 0'''
    
    mean_x = torch.mean(x)
    mean_y = torch.mean(y)
    stddev_x = torch.std(x)
    stddev_y = torch.std(y)
    NCC = torch.sum((x-mean_x)*(y-mean_y)/(stddev_x*stddev_y))/(config.iWidth**3)
    NCC[ ~ torch.isfinite( NCC )] = 0
    return 1 - NCC   # torch.mean(MeanMatrix)
